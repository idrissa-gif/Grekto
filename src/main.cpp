...
const string PROGRAM_NAME = "GreKo 2021.08 NNUE";
const string AUTHOR_NAME = "Vladimir Medvedev, Daniel Shawul, Stockfish development team";
const string RELEASE_DATE = "15-Aug-2021";

string g_weightsFile = "weights.txt";
string g_nnueFile = "network.nnue";
...
void OnSetoption()
{
	...
	else if (name == "NNUE")
	{
		...
		{
			g_useNNUE = true;
			nnue_init(g_nnueFile.c_str());
			Log("Using NNUE file: %s\n", g_nnueFile.c_str());
		}
		else
			g_useNNUE = false;
	}
...
void OnUCI()
{
	...
	Out("option name NNUE type check default true\n");
	Out("option name Network type string default network.nnue\n");
