...
static void half_kp_append_active_indices(const NNUEPosition *pos, const int c,
    IndexList *active)
{
...
static void half_kp_append_changed_indices(const NNUEPosition *pos, const int c,
...
static void append_active_indices(const NNUEPosition *pos, IndexList active[2])
{
  for (unsigned c = 0; c < 2; c++)
    half_kp_append_active_indices(pos, c, &active[c]);
}

static void append_changed_indices(const NNUEPosition *pos, IndexList removed[2],
...
INLINE void refresh_accumulator(NNUEPosition *pos)
{
...
INLINE bool update_accumulator(NNUEPosition *pos)
{
  Accumulator *accumulator = &(pos->nnue[0]->accumulator);
...
INLINE void transform(NNUEPosition *pos, clipped_t *output, mask_t *outMask)
{
