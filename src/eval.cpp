...
EVAL Eval_NNUE(const Position& pos)
{
	static const int nnue_pieces[14] =
	{
		0,
		0,
		6,   // PW
	...
	static const int nnue_fields[64] =
	{
	...
	pieces[0] = nnue_pieces[KW];
	squares[0] = nnue_fields[pos.King(WHITE)];

	pieces[1] = nnue_pieces[KB];
	squares[1] = nnue_fields[pos.King(BLACK)];

	int i = 2;
	for (FLD f = 0; f < 64; ++f)
	{
		PIECE p = pos[f];
		if (p == NOPIECE || p == KW || p == KB) continue;

		pieces[i] = nnue_pieces[p];
		squares[i] = nnue_fields[f];
		i += 1;
	}
	pieces[i] = 0;

	return nnue_evaluate(pos.Side(), pieces, squares);
}
////////////////////////////////////////////////////////////////////////////////
...
EVAL Evaluate(const Position& pos, EVAL alpha, EVAL beta)
{
	...
	e = (EVAL)DotProduct(pos.Stage(), score);
